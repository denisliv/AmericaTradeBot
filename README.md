...
- **✅ Подобрать авто из США**
  ...
  - *помощь в выборе*: тип кузова → бюджет → ТОП-3 примера из локальной галереи
    с кнопками расчёта; порядок задан приоритетами заказчика, «Подобрать еще»
    выдаёт следующие три без повторов.
  - на каждом шаге выбора критериев есть кнопка «🔙 Назад».
...
├── scripts/                      # Проверка рассылок и подготовка галереи
...
| `data/assisted_gallery/**` | `data/assisted_gallery/` | галерея примеров: `<кузов>/<бюджет>/<приоритет>_<марка_модель>/*.jpg` + `gallery.json` (sedan/suv/electric × 6 бюджетов) |
...
## Обновление галереи «помощь в выборе»

Галерея собирается из выгрузки Google Drive заказчика: папки вида
`1. Кроссовер / 2. До 15.000 / 12. 2022 Mitsubishi Eclipse Cross`. Номер папки —
это приоритет показа, в таком порядке бот и выдаёт авто.

```bash
# 1. Распаковать выгрузку Drive в data/gdrive (три папки кузовов)
# 2. Собрать галерею: слаги папок, манифест, AVIF -> JPEG (Telegram не принимает AVIF)
uv run python scripts/import_gallery.py --force

# 3. Убрать студийные снимки на белом фоне (сначала отчёт, потом удаление)
uv run python scripts/filter_white_bg.py
uv run python scripts/filter_white_bg.py --apply
```

`import_gallery.py` создаёт `data/assisted_gallery/gallery.json` — приоритет,
полное название и число фото каждого авто. Названия из манифеста показываются
клиенту как есть, поэтому опечатки Drive правятся прямо в нём: при повторном
запуске импорта правки сохраняются.

`filter_white_bg.py` не оставляет авто без фото: если после чистки в папке
осталось бы меньше 5 снимков, папка не трогается и попадает в отчёт — такие
папки чистятся вручную.

После ручной правки фото манифест нужно привести в соответствие с файлами,
иначе бот будет обещать авто и снимки, которых уже нет:

```bash
uv run python scripts/sync_gallery_manifest.py           # что разошлось
uv run python scripts/sync_gallery_manifest.py --apply   # пересчитать
```

Обе задачи выполняются локально; на сервер уезжает уже готовая `data/assisted_gallery`.
