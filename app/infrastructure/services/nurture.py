...
шаг 1 - через 60 минут, далее по посту каждый день (дни 1-7) в 12:00 по таймзоне бота.
...
def resolve_step(last_step: int) -> tuple[int, int]:
    ...
    return next_step, _SOCIAL_STEPS[(next_step - 9) % 3]
...
def due_at(
    ...
    if step <= 8:
        ...
        content_step = _SOCIAL_STEPS[(step - 9) % 3]
        cycle = (step - 9) // 3 + 1
...
async def _send_random_cars_step(bot: Bot, user_id: int, first_name: str) -> None:
    """Шаг 4: случайный кроссовер и седан из CSV одним выпуском."""
    cars = []
    for body_group in ("suv", "sedan"):
        car = await get_random_car_with_images(body_group)
        if car:
            cars.append(car)

    if not cars:
        logger.warning("Nurture step 4: no cars with images found, skipping content")
        await _send_text_step(bot, user_id, LEXICON_NURTURE_RU["thinking_text"])
        return

    await bot.send_message(
        chat_id=user_id,
        text="🔥 <b>ТОП авто из США - подготовили подборку специально для вас</b>",
    )
    for number, car in enumerate(cars, 1):
        media_group = await make_media_group(car, first_name, number)
                ...
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text=f"📋 Получить детальный расчет Авто № {number}",
                            callback_data=(
...
async def send_nurture_step(
    ...
    if content_step == 1:
        await _send_text_step(bot, user_id, LEXICON_RU["why_americatrade_text"])
    elif content_step == 2:
        await _send_text_step(bot, user_id, LEXICON_NURTURE_RU["myths_text"])
    elif content_step == 3:
        await _send_text_step(bot, user_id, LEXICON_NURTURE_RU["client_story_text"])
    elif content_step == 4:
        await _send_random_cars_step(bot, user_id, first_name)
    elif content_step == 5:
        await _send_text_step(bot, user_id, LEXICON_NURTURE_RU["thinking_text"])
    elif content_step == 6:
        await bot.send_message(
